...
func (s *service) Create(ctx context.Context, r *taskAPI.CreateTaskRequest) (_ *taskAPI.CreateTaskResponse, err error) {
	...
	ctx, bundleLoadSpan := shimTracer.Start(ctx, "shim.BundleLoad")
	var (
	...
	ctx, vmBootSpan := shimTracer.Start(ctx, "shim.VMBoot")
	prestart := time.Now()
	...
	ctx, bundleTransferSpan := shimTracer.Start(ctx, "shim.BundleTransfer")
	...
	ctx, ioSetupSpan := shimTracer.Start(ctx, "shim.IOSetup")
	rio := stdio.Stdio{
		Stdin:    r.Stdin,
		Stdout:   r.Stdout,
		Stderr:   r.Stderr,
